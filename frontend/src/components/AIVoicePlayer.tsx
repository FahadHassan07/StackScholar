...
export const AIVoicePlayer: React.FC<AIVoicePlayerProps> = ({
  ...
  const generateSpeech = async () => {
    ...
    try {
      ...
      const response = await apiPost('/ai/speak', {
        text: text.substring(0, 500), // Limit to 500 chars to avoid long generation
        voice,
        format: 'mp3',
      });
      ...
      Alert.alert('Error', error.message || 'Failed to generate speech. Check if OpenAI API key is configured.');
    } finally {
      setIsLoading(false);
    }
  };
